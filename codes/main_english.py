...
f = open('data/eng_50_sent.txt', 'r')
data_en = f.readlines()
f.close()
#data = data.replace('.', '')
#data = data.replace("\n", '. ').replace("\r", ". ")
#doc = nlp_en("Barack Obama was born in Hawaii.  He was elected president in 2008.")
f = open('english_depdency_50.txt', 'w', encoding = 'utf-8')
for sent in data_en:
	doc_en = nlp_en(sent)
	...
	for output in get_dependencies(doc_en, 1):
		...
		f.write('============\n')
f.close()
print("Completed english dependency")

# this prints the POS-tagged sentence in a human-readable format

f = open('english_pos_50.txt', 'w', encoding = 'utf-8')
for sent in data_en:
	doc_en = nlp_en(sent)
	# this prints the dependencies in a human-readable format	
